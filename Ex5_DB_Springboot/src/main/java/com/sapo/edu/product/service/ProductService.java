...
//        }
    ...
    @Override
    public List<Product> fillAllProductPaging(int page, String query, int pageSize) throws Exception {
        double total_item = productDAOInterface.getTotalProductCount();
        int total_page = (int) Math.floor(total_item / pageSize);
        if (page > total_page || page < 1)
            throw new Exception("Max page range input is: 1 -> " + total_page);
        return productDAOInterface.fillAllProductPaging(page, query, pageSize);
    }
    ...
    public String insertProduct(Product product) throws Exception {
        ...
        result += "\nYou have switchFunction " + productDAOInterface.insertProduct(product) + " Product";
        return result;
    }
    ...
    public String insertListProduct(List<Product> productList) throws Exception {
        ...
        result += "\nYou have switchFunction " + productDAOInterface.insertListProduct(productList) + " Product";
        return result;
    }
