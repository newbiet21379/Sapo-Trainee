...
public class ProductDAO implements ProductDAOInterface {
    ...
    @Override
    public List<Product> fillAllProductPaging(int page, int pageSize,int id) {
        //TODO viết câu lệnh sql lấy danh sách product theo name và có paging;
        
        String sql = "SELECT * FROM product p where category_id=?  Limit ?,?";
        System.out.println(sql);
        return this.jdbcTemplate.query(sql, new ProductMapper.ProductRowMapper(),id, page * pageSize, pageSize);
    ...
    @Override
    public int getTotalProductCountID(int id) {
        String sql="select * from product where category_id=?";
        RowCountCallbackHandler countCallback = new RowCountCallbackHandler();  // not reusable
        jdbcTemplate.query(sql, countCallback,id);
        return countCallback.getRowCount();
    }
    /**
     * Thêm sản phẩm
