...
public class CategoryService implements CategoryServiceInterface {
    ...
    @Override
    public ResponseForm fillAllCategoryPaging(int page,String query, int pageSize) throws Exception {

        double total_item = categoryDAOInterface.getTotalCategory(query);
        int total_page = (int) Math.ceil(total_item / pageSize);
        if (page > total_page-1 || page < 0)
            throw new Exception("Max page max range input is: " + total_page);

        return new ResponseForm(categoryDAOInterface.fillAllCategoryPaging(page,query, pageSize),total_page);
    }
    ...
    public int getTotalCategory(String query) {
        return categoryDAOInterface.getTotalCategory(query);
    }
    ...
    public String insertCategory(Category category) throws Exception {
        ...
        result += "\nYou have add " + categoryDAOInterface.insertCategory(category) + " Category";
        return result;
    }
